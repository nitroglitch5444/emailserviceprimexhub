...
import { Settings, Package, ShoppingBag, Mail, ShieldAlert, Database, Cpu, Monitor, Wifi, WifiOff } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTabState] = useState(() => {
    ...
    return ['config', 'products', 'orders', 'emails', 'stocking', 'devices'].includes(hash) ? hash : 'config';
  });
  ...
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#', '');
      if (['config', 'products', 'orders', 'emails', 'stocking', 'devices'].includes(hash)) {
