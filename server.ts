...
const automationStateSchema = new mongoose.Schema({
  command: { type: String, default: 'idle' },
...
async function startServer() {
  ...
  app.get('/api/admin/automation/status', authenticateToken, isAdmin, async (req, res) => {
    let state = await AutomationState.findOne();
    if (!state) state = await AutomationState.create({ command: 'idle', isRunning: false });
    res.json({ command: state.command, isRunning: state.isRunning });
  });

  app.post('/api/admin/automation/command', authenticateToken, isAdmin, async (req, res) => {
    const { command } = req.body;
    let state = await AutomationState.findOne();
    if (!state) state = new AutomationState();
    ...
    await state.save();
    res.json({ success: true, command: state.command });
  });
  ...
  app.post('/api/automation/report', async (req, res) => {
    ...
    const state = await AutomationState.findOne() || new AutomationState();
    if (status) state.command = status;
    if (typeof isRunning === 'boolean') state.isRunning = isRunning;
    await state.save();
    
    res.json({ command: state.command });
  });
